...
def calculates_results_stats(results_dic):
    ...
    for key, lst in results_dic.items():
        if(lst[2]==1):
                n_match+=1
        if(lst[3]==1):n_dog_img+=1
        else : n_notdog_img+=1
        if(lst[2]==1 and lst[3]==1):
                n_correct_breed+=1
        if(lst[3]==1 and lst[4]==1):
                n_correct_dogs+=1
        if(lst[3]==0 and lst[4]==0):
                n_correct_notdogs+=1
    ...
    results_stats_dic['n_notdogs_img']=n_notdog_img
