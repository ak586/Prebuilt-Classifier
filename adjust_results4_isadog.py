...
def adjust_results4_isadog(results_dic, dogfile):
    ...
    with open(dogfile, "r") as infile:
        ...
        while line:
            ...
            for dog_names in curr_line:
                if(dognames_dic.get(dog_names)==None):
    ...
    for key in results_dic:
        predicted_list=results_dic[key][1].split(',')
        formatted_list=[x.lower().strip() for x in predicted_list]
        
        if results_dic[key][0].strip() in dognames_dic:
            found=False
            for classified_names in formatted_list:
                if(dognames_dic.get(classified_names)!=None):
